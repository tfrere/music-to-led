...
class ShellInterface():
    ...
    def printHeader(self, y):
        ...
        # add STATES nb
        self.absolutePrint(y + 2, space, "Strips: " + str(len(self.config.strips)) + " | audio source: " + str(self.config.number_of_audio_ports) + " | states : " + str(self.config.number_of_states))
        self.absolutePrint(y + 4, space, "Shell dimensions: " + str(self.width) + "x" + str(self.height))
        self.absolutePrint(y + 5, space, "Memory consumed: " + str(self.getConsumedVirtualMemory()) + " Key input: " + self.input)
    ...
    def drawBox(self, offset, size, color=(255,255,255)):
        ...
        first_line = self.box_chars[2] + (self.box_chars[0] * (w - 2)) + self.box_chars[3]
        middle_line = self.box_chars[1] + (" " * (w - 2)) + self.box_chars[1]
        last_line = self.box_chars[4] + (self.box_chars[0] * (w - 2)) + self.box_chars[5]

        self.absolutePrint(y, x, self.textWithColor(r,g,b,first_line))
        for i in range(h):
            self.absolutePrint(y + i + 1, x, self.textWithColor(r,g,b,middle_line))
    ...
    def printAudio(self, y, x, name, audio_datas):
        ...
        if(self.has_to_draw_static_components):
            ...
            self.absolutePrint(y + 2, x - 1, self.textWithColor(self.rgb_inner_border_color[0],self.rgb_inner_border_color[1], self.rgb_inner_border_color[2], "├" + ("─" * (29 - 2)) + "┤"))
        ...
        for channel in audio_datas:
            charIndex = int(channel * 10) if int(channel * 10) < len(self.audio_viz_chars) - 1 else len(self.audio_viz_chars) - 1
    ...
    def printStrip(self, y, is_connected, framerate, strip_config, active_state, pixels):
        ...
        if(self.has_to_draw_static_components):
            ...
            self.drawBox(offset, size, self.rgb_border_color)
            self.absolutePrint(offset[1] + 2, 0, self.textWithColor(self.rgb_inner_border_color[0],self.rgb_inner_border_color[1], self.rgb_inner_border_color[2], "├" + ("─" * (self.min_width - 2)) + "┤"))
            self.absolutePrint(offset[1] + 7, 0, self.textWithColor(self.rgb_inner_border_color[0],self.rgb_inner_border_color[1], self.rgb_inner_border_color[2], "├" + ("─" * (self.min_width - 2)) + "┤"))

        is_connected = self.textWithColor(0, 255, 0, ' online ⬤ ') if is_connected else self.textWithColor(255, 0, 0, ' offline ⬤ ')
        is_connected_str = str(framerate) + self.textWithColor(100, 100, 100, ' FPS ') + self.textWithColor(100, 100, 100, ' | ') + is_connected
        ...
        if(strip_config.midi_logs and strip_config.midi_logs[0]["port"] and strip_config.midi_logs[0]["note"]):
            midi_note_to_print = str(strip_config.midi_logs[0]["port"]) + self.textWithColor(100, 100, 100, " note played : ") + str(strip_config.midi_logs[0]["note"])
            len_midi_note_to_print = len(str(strip_config.midi_logs[0]["port"]) + " note played : " + str(strip_config.midi_logs[0]["note"]))
            self.absolutePrint(y + 3, self.width - len_midi_note_to_print - 2, "                                             ")
            self.absolutePrint(y + 3, self.width - len_midi_note_to_print - 2, midi_note_to_print)

 
        self.absolutePrint(y + 4, 0, self.textWithColor(self.rgb_inner_border_color[0],self.rgb_inner_border_color[1], self.rgb_inner_border_color[2], "├" + ("─" * (self.min_width - 2)) + "┤"))

        self.absolutePrint(y + 5, 2, self.textWithColor(100, 100, 100, 'audio channel'))
        self.absolutePrint(y + 6, 2, "                    ")
        self.absolutePrint(y + 6, 2, self.config.audio_ports[active_state.active_audio_channel_index].name)

        self.absolutePrint(y + 5, 24, self.textWithColor(100, 100, 100, 'color scheme'))
        self.absolutePrint(y + 6, 24, "                    ")
        self.absolutePrint(y + 6, 24, color_scheme)

        self.absolutePrint(y + 5, 42, self.textWithColor(100, 100, 100, 'shape'))
        self.absolutePrint(y + 6, 42, "                           ")
        self.absolutePrint(y + 6, 42, shape)

        self.absolutePrint(y + 5, 69, self.textWithColor(100, 100, 100, 'time_interval'))
        self.absolutePrint(y + 6, 69, "                    ")
        self.absolutePrint(y + 6, 69, str(active_state.time_interval))

        self.absolutePrint(y + 5, 85, self.textWithColor(100, 100, 100, 'brightness'))
        self.absolutePrint(y + 6, 85, "                    ")
        self.absolutePrint(y + 6, 85, str(active_state.max_brightness))

        self.absolutePrint(y + 5, 97, self.textWithColor(100, 100, 100, 'chunk_size'))
        self.absolutePrint(y + 6, 97, "                    ")
        self.absolutePrint(y + 6, 97, str(active_state.chunk_size))

        self.absolutePrint(y + 5, 112, self.textWithColor(100, 100, 100, 'blur_value'))
            ...
            if(i >= chunk_offset + chunk_length - 1):
                ...
                if(j == 0 and total_number_of_lines == 0):
                    ...
                    if(i < len(pixels[0])):
                        ...
                        array += self.textWithColor(pixels[0][i], pixels[1][i], pixels[2][i], self.pixel_char)
                    else:
    ...
    def drawFrame(self, shared_list):
        ...
        with self.term.hidden_cursor(), self.term.cbreak():
            ...
            if(self.is_below_min_width):
                ...
                self.absolutePrint(self.height // 2, self.width // 2 - (len(minWidthSentence) // 2), minWidthSentence)
            else:
